...
def example_1_or_filter():
    ...
    results = Works().filter_or(
        display_name_search=["deep learning", "neural networks"]
    ).get(limit=5)
...
def example_2_range_filter():
    ...
    results = Works().filter_gt(
        cited_by_count=100
    ).filter(publication_year=2020).get(limit=5)
...
def example_3_not_filter():
    ...
    results = Works().search("artificial intelligence").filter_not(
